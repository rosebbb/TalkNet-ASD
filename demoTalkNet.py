...
def track_shot(args, sceneFaces):
    ...
    while True:
        ...
        # print(x)
        track     = []
        for iframe, frameFaces in enumerate(sceneFaces):
            # print(f'frame {iframe} has {len(frameFaces)} faces')
            for iface, face in enumerate(frameFaces):
                # print(f'--face {iface}')
                if track == []:
                    # print(f'----track is 0')
                    track.append(face)
                    frameFaces.remove(face)
                elif face['frame'] - track[-1]['frame'] <= args.numFailedDet: ### Design
                    # print(f'----compare with last face in track')
                    iou = bb_intersection_over_union(face['bbox'], track[-1]['bbox'])
                    if iou > iouThres:
                        # print(f'------overlap, add to track')
                        track.append(face)
                        frameFaces.remove(face)
                        continue
                else:
...
def crop_video(args, track, cropFile):
    ...
    for det in track['bbox']: # Read the tracks
        dets['s'].append(max((det[3]-det[1]), (det[2]-det[0]))/2) # half of the larger dimention(if h>w, this equals to h/2)
...
def evaluate_network(files, args, durationSet):
    ...
    for file in tqdm.tqdm(files, total = len(files)):
        ...
        while video.isOpened():
            ...
            if ret == True:
                ...
                cv2.imwrite(f'temp/{fileName}/ori_{x}.jpg', face)
                face = face[int(112-(112/2)):int(112+(112/2)), int(112-(112/2)):int(112+(112/2))]
                #face = cv2.resize(face, (112,112))

                cv2.imwrite(f'temp/{fileName}/used_{x}.jpg', face)
                x+=1
        ...
        length = min((audioFeature.shape[0] - audioFeature.shape[0] % 4) / 100, videoFeature.shape[0]) # audio and video length may not be perfectly aligned. Use the shorter one to crop them
        audioFeature = audioFeature[:int(round(length * 100)),:]
        videoFeature = videoFeature[:int(round(length * 25)),:,:]
        allScore = [] # Evaluation use TalkNet
        for duration in durationSet:
            batchSize = int(math.ceil(length / duration)) # d
            scores = []
            with torch.no_grad():
        ...
        # plt.plot(range(len(allScore)), allScore, label = "Average")
