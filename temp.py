...
def main():
    ...
    # dummy_image = cv2.putText(dummy_image, '<ESC>   : Exit program', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
    # dummy_image = cv2.putText(dummy_image, '<SPACE> : Enabling / Disabling noise suppression', (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
    # cv2.imshow('Real time demo', dummy_image)
    ...
    #print('input_shapes: ', input_shapes["input"])
    # res = None
    ...
    input_device_index = int(input("Enter Input Device ID: "))
    output_device_index = int(input("Enter Output Device ID: "))

    # get_format_from_width(2), paInt16
    record_stream   = audio.open(format=pyaudio.paInt16, channels=1, rate=16000, input =True, frames_per_buffer=input_size, input_device_index = input_device_index)
    playback_stream = audio.open(format=pyaudio.paInt16, channels=1, rate=16000, output=True, frames_per_buffer=input_size, output_device_index = output_device_index)
    ...
    while key != 27:
        ...
        input_audio = np.frombuffer(record_stream.read(num_frames=input_size), dtype=np.int16) # exception_on_overflow = False
        if args.audio_log:
            wav_in.writeframes(input_audio.tobytes())     # record input audio
        normalized_input = input_audio.astype(np.float32) * (1.0 / np.iinfo(np.int16).max)
 
        #forms input
        inputs = {"input": normalized_input[None, :]}
        #time.sleep(5)
        #print("inputs: ", inputs.keys())
        #print("state_inp_names: ", state_inp_names)
        ...
        #print("inputs with states: ", inputs.keys())
        #print("res: ", res.keys() if res else None)
        ...
        #print("delay: ", res['delay'].buffer[0])
